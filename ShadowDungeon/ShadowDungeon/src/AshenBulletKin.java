...
/**
 * Variation of enemy that shoots faster and has more health
 */
public class AshenBulletKin extends Enemies {
    ...
    /**
     * Constructor for AshBulletKin enemy
     * @param pos Position of the enemy to determine where it spawns.
     */
    public AshenBulletKin(Point pos){
        this.image = new Image("res/ashen_bullet_kin.png");
        position =pos;
        this.x=position.x;
        this.y=position.y;
        this.health=Double.parseDouble(ShadowDungeon.gameProps.getProperty("ashenBulletKinHealth"));
        this.SHOOTING_RATE= Double.parseDouble(ShadowDungeon.gameProps.getProperty("ashenBulletKinShootFrequency"));
        this.EARNED_COINS=Double.parseDouble(ShadowDungeon.gameProps.getProperty("ashenBulletKinCoin"));
    }
    ...
    public void update(Player player){
        //Stop updating if enemies is already dead
        if (dead){
            return;
        }
